...
TEST_CASE("combineModeOrder lists AutoPerEar first with the right recommended badges") {
    auto order = eb::combineModeOrder();
    REQUIRE (order.size() == 5);
    CHECK (order[0].mode == eb::CombineMode::AutoPerEar);   // recommended, on top
    CHECK (order[1].mode == eb::CombineMode::LeftOnly);
    CHECK (order[2].mode == eb::CombineMode::RightOnly);
    CHECK (order[3].mode == eb::CombineMode::Average);
    CHECK (order[4].mode == eb::CombineMode::Sum);
    // Only AutoPerEar carries the Recommended badge.
    CHECK (order[0].recommended == true);    // AutoPerEar
    CHECK (order[1].recommended == false);   // LeftOnly
    CHECK (order[2].recommended == false);   // RightOnly
    CHECK (order[3].recommended == false);   // Average
    CHECK (order[4].recommended == false);   // Sum
