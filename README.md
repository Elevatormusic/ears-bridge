...
- **Windows** — built and tested with the bundled test suite. This is the primary, verified platform.
- **macOS** — builds from the same CMake project and uses CoreAudio (including an optional private
  ...
  still needs validation against real hardware; see the bench-validation runbook below.

There are no pre-built binaries yet. Install is "build from source," documented below.
...
- A C++20 toolchain to build from source (see below).

## Installation

### 1. Install a virtual audio device

Install VB-CABLE (Windows) or BlackHole 2ch (macOS) and reboot if the installer asks. EARS Bridge
sends its mono output to this device; Dirac reads from the device's capture side.
...
```
build\EarsBridge_artefacts\Release\EARS Bridge.exe
```
