...
- [ ] **MANUAL VERIFICATION (in Task 9 window):**
  - Drag `R_HPN_0000000.txt` onto the "Left ear cal" slot. PASS = filename shows, detail reads `Serial 000-0000  -  HPN`, the thumbnail draws a curve with a visible dip near 4 kHz (the loaded HPN is the *cut* curve), no warning badge.
...
- [ ] **MANUAL VERIFICATION — full window (launch `build/EarsBridge_artefacts/Release/EARS Bridge.exe`, or the `.exe` under `build/`):**
  ...
  5. **Cal slots (Task 6):** drag `R_HPN_0000000.txt` onto each slot; detail reads `Serial 000-0000  -  HPN`; thumbnail draws the curve with the 4 kHz feature; no badge. Drag an HEQ file → orange `HEQ - double-target?` badge appears. PASS as Task 6 block.
