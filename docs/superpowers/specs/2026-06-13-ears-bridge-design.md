...
1. **EARS is fixed 24-bit / 48 kHz, UAC1, driverless.** The original EARS (serial 000-0000) cannot capture at any other rate. Higher rates are physically impossible on this device; selecting them would resample the 48 kHz capture, which only *adds* error. EARS *Pro* (XMOS/UAC2, 44.1–192 kHz) is a different product.
